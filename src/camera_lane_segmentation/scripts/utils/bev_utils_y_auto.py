...
Modifications:
- The user manually selects all 4 points required for BEV setup.
- The y-coordinates of the right points (right-bottom, right-top) are automatically aligned with the y-coordinates of the left points.
...
def mouse_callback(event, x, y, flags, param):
    ...
    The y-coordinate of the right points is automatically aligned to be the same as the left points.
    """
    global src_points
    if event == cv2.EVENT_LBUTTONDOWN:
        if len(src_points) < max_points:
            ...
            # Automatically align the y-coordinates for the 2nd and 4th points
            if current_point_index == 1:   # Time to click the right-bottom point
                y_bottom = src_points[0][1] # Get the y-coordinate of the first point (left-bottom)
                final_point = (x, y_bottom)
            elif current_point_index == 3: # Time to click the right-top point
                y_top = src_points[2][1]    # Get the y-coordinate of the third point (left-top)
                final_point = (x, y_top)
...
def main():
    ...
    print("       ✨ The right points will be automatically aligned to the y-coordinates of the left points.")
